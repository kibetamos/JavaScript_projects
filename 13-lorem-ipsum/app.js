...
`Proin vel bibendum nulla. Etiam vitae felis a nulla tempor convallis. Sed facilisis suscipit diam, nec efficitur enim gravida nec. Phasellus eget sapien massa. Nulla facilisi. Suspendisse potenti. Curabitur in tortor non leo facilisis hendrerit. Cras volutpat, ante id laoreet efficitur, purus felis facilisis erat, vel scelerisque metus arcu in leo. Etiam luctus velit in nulla congue, eget imperdiet nisl congue.`
];
...
//listem to an event on the form
 ...
 const random = Math.floor(Math.random() * text.length);
  ...
  if(isNaN(value) || value <= 0 || value > 9){
    result.innerHTML = `<p class="result"> ${text[random]}</p>`
      ...
      const random = Math.floor(Math.random() * text.length);
      tempText.push(`<p class="result">${text[random]}</p>`);
  }
